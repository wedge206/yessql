    ...
    public sealed class CreateIndexCommand : IndexCommand
    {
        ...
        public override async Task ExecuteAsync(DbConnection connection, DbTransaction transaction, ISqlDialect dialect, int timeout, ILogger logger)
        {
            ...
            if (Index is MapIndex)
            {
                ...
                logger.LogTrace(sql);
                Index.Id = await connection.ExecuteScalarAsync<int>(sql, Index, transaction, timeout);
                ...
                await connection.ExecuteAsync(command, new { mapid = Index.GetAddedDocuments().Single().Id, Id = Index.Id }, transaction, timeout);
            }
            else
            {
                ...
                await connection.ExecuteAsync(bridgeSql, _addedDocumentIds.Select(x => new { DocumentId = x, Id = Index.Id }), transaction, timeout);
            }
        }
    }
}
